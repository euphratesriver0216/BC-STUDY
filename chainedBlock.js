...
function hasMatchesDifficulty(hash, difficulty) {
  ...
  return hashBinary.startsWith(requirePrefix); //앞자리가 requirePrefix로 시작하는 함수가 된다
}
...
function findBlock(
  ...
  while (true) {
    ...
    nonce++; //true가  아닐경우 넌스값을 1씩 증가해가면서 비교해준다.
  }
}
...
function getAdjustDifficulty(lastBlock, blocks) {
  const prevAdjustmentBlock =
    blocks[blocks.length - DIFFICULTY_ADJUSTMENT_INTERVAL]; //블럭단위로 난이도가 조절
  const elapsedTime =
